...
# Extract data from database using SQL query above
deaths <- odbc::dbGetQuery(smra_connect, Query_Deaths) %>%
  tibble::as_tibble() %>% 
  clean_names()
...
deaths <- filter(deaths, externalcause == 0 & falls == 0 | falls == 1)

deaths <- deaths %>% mutate(
  fy = case_when(
    (date_of_death >= "2013-04-01" & date_of_death < "2014-04-01") ~ "2013/14",
    (date_of_death >= "2014-04-01" & date_of_death < "2015-04-01") ~ "2014/15",
    (date_of_death >= "2015-04-01" & date_of_death < "2016-04-01") ~ "2015/16",
    (date_of_death >= "2016-04-01" & date_of_death < "2017-04-01") ~ "2016/17",
    (date_of_death >= "2017-04-01" & date_of_death < "2018-04-01") ~ "2017/18",
    (date_of_death >= "2018-04-01" & date_of_death < "2019-04-01") ~ "2018/19",
...
# match on deaths
smr_deaths <- left_join(smr, deaths, by = "link_no") %>% 
...
agg_hosp_los <- hosp_los_wider %>% group_by(fy, ca, link_no) %>%
...
# combine
output_all <- rbind(output_lca, output_sc, output_scot)
...
output_final <- output_all %>%
  ...
  select(finyear, lca, large_beddays, community_hosp_beddays, palliative_beddays,
  ...
  select(finyear, lca, community_beddays, palliative_beddays, community_hosp_beddays, 
         large_beddays, possible_beddays, perc_community, perc_palliative, perc_community_hosp,
         perc_large, deaths) %>%
  arrange(finyear, lca) %>%
  
  # amend FY & lca to match excel template
  mutate(
    finyear = str_c(str_sub(finyear, 1, 5), "20", str_sub(finyear, 6, 7)),
    lca = recode(lca, 
